...
test('Hermes setup merges native pre/post hooks without replacing existing YAML', () => {
  ...
  assert.equal(result.hooks.pre_tool_call[0].command, 'npx -y pilotprotocol-mcp@0.2.10 hook --harness hermes --phase pre');
  assert.equal(result.hooks.post_tool_call.length, 1);
  assert.deepEqual(result.mcp_servers.pilot.args, ['-y', 'pilotprotocol-mcp@0.2.10']);
});
